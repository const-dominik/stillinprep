...
import { MovePopualritySettings } from "@/app/types/types";

jest.mock("@/app/actions/repertoire", () => ({
    updateRepertoireField: jest.fn(),
}));

jest.mock("@/app/hooks/useChange", () => ({
    __esModule: true,
    default: (callback: () => void, deps: any[]) => {
        callback();
    },
}));

import LichessDbSettings from "@/app/components/repertoire/LichessDbSettings";
